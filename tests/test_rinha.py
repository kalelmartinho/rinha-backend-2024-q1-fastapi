...
@pytest.mark.asyncio
async def test_transacao_saldo_insuficiente(client: AsyncClient) -> None:
    """Testa se a rota de transação retorna 422 para saldo insuficiente."""
    payload = {"valor": 1000000, "tipo": "d", "descricao": "descricao"}
    response = await client.post("/clientes/1/transacoes", json=payload)
